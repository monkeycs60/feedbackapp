...
test.describe('Onboarding Flow', () => {
	...
	test('should display role selection page for new users', async ({
		page,
	}) => {
		...
		await expect(
			page.locator('text=Comment veux-tu commencer ?')
		).toBeVisible();
		...
		await expect(
			page.locator(
				'[data-testid="creator-card"] >> text=J\'ai une app à faire roaster'
			)
		).toBeVisible();
		await expect(
			page.locator('[data-testid="creator-card"] >> text=✓ Feedback en 24h')
		).toBeVisible();
		await expect(
			page.locator('[data-testid="creator-card"] >> text=✓ À partir de 5€')
		).toBeVisible();

		// Check roaster card content
		await expect(
			page.locator(
				'[data-testid="roaster-card"] >> text=Je veux gagner de l\'argent'
			)
		).toBeVisible();
		await expect(
			page.locator(
				'[data-testid="roaster-card"] >> text=✓ 3.50€ par feedback'
			)
		).toBeVisible();
		await expect(
			page.locator(
				'[data-testid="roaster-card"] >> text=✓ Choisis tes missions'
			)
		).toBeVisible();

		// Check reassurance message
		await expect(
			page.locator('text=💡 Tu pourras facilement switcher plus tard')
		).toBeVisible();
	});
	...
	test('should complete roaster onboarding flow', async ({ page }) => {
		...
		await expect(page.locator('[data-testid="roaster-card"]')).toHaveClass(
			/ring-2/
		);
		...
		await expect(
			page.locator(
				'text=Quelques infos pour matcher avec les bonnes missions'
			)
		).toBeVisible();
		...
		await expect(page.locator('[data-testid="specialty-UX"]')).toHaveClass(
			/border-orange-500/
		);
		await expect(page.locator('[data-testid="specialty-Dev"]')).toHaveClass(
			/border-orange-500/
		);
		...
		await expect(
			page.locator('[data-testid="experience-Expert"]')
		).toHaveClass(/border-orange-500/);

		// Fill optional bio
		await page.fill(
			'[data-testid="bio-textarea"]',
			"UX Designer avec 5 ans d'expérience dans les SaaS B2B"
		);
		...
		await expect(
			page.locator('text=Tu es maintenant prêt à monétiser ton expertise')
		).toBeVisible();

		// Check next steps for roaster
		await expect(
			page.locator('text=Explore les missions disponibles')
		).toBeVisible();
		await expect(
			page.locator('text=Postule à celles qui te correspondent')
		).toBeVisible();
		await expect(
			page.locator('text=Gagne tes premiers 3.50€ !')
		).toBeVisible();

		// Check future role preview
		await expect(
			page.locator('text=Une fois que tu auras donné quelques feedbacks')
		).toBeVisible();
	...
	test('should complete creator onboarding flow', async ({ page }) => {
		...
		await expect(page.locator('[data-testid="creator-card"]')).toHaveClass(
			/ring-2/
		);
		...
		await expect(page.locator('h1')).toContainText('Finalise ton profil');
		await expect(
			page.locator('text=Quelques infos pour personnaliser ton expérience')
		).toBeVisible();
		...
		await expect(
			page.locator(
				'text=Tu es maintenant prêt à obtenir des feedbacks brutaux'
			)
		).toBeVisible();

		// Check next steps for creator
		await expect(
			page.locator('text=Poste ta première app à roaster')
		).toBeVisible();
		await expect(
			page.locator('text=Choisis tes roasters (à partir de 5€)')
		).toBeVisible();
		await expect(
			page.locator('text=Reçois des feedbacks en 24h')
		).toBeVisible();

		// Check future role preview
		await expect(
			page.locator('text=Une fois que tu auras reçu des feedbacks')
		).toBeVisible();
	...
	test('should redirect incomplete onboarding users', async ({ page }) => {
		// User is already signed up but hasn't completed onboarding
	...
	test('should show validation errors for incomplete roaster profile', async ({
		page,
	}) => {
		// Go to profile setup
		await page.goto('/onboarding/role-selection');
		...
		await expect(
			page.locator('text=Sélectionne au moins une spécialité')
		).toBeVisible();
	...
	test('should allow role switching hint', async ({ page }) => {
		...
		await expect(
			page.locator('text=Bientôt disponible pour toi')
		).toBeVisible();
	});
});
...
test.describe('Onboarding Accessibility', () => {
	...
	test('should have proper ARIA labels', async ({ page }) => {
		...
		await expect(
			page.locator('[data-testid="creator-card"]')
		).toHaveAttribute('role', 'button');
		await expect(
			page.locator('[data-testid="roaster-card"]')
		).toHaveAttribute('role', 'button');
	});
});
