...
import { completeOnboarding } from '../helpers/auth-helper';
...
test.describe('Authentication Flow', () => {
	test('should display navbar with login/signup buttons when not authenticated', async ({
		page,
	}) => {
		...
		await expect(
			page.getByRole('link', { name: 'RoastMyApp' })
		).toBeVisible();
	...
	test('should create a new account', async ({ page }) => {
		...
		await page.waitForURL('/onboarding/role-selection');
	...
	test('should login with existing account', async ({ page }) => {
		...
		// Should go to dashboard since onboarding is complete
	...
	test('should show error for invalid credentials', async ({ page }) => {
		...
		await expect(
			page.getByText(/Authentication failed|Invalid credentials/)
		).toBeVisible();
	});
	...
	test('should access profile page when authenticated', async ({ page }) => {
		...
		await expect(
			page.getByRole('definition').filter({ hasText: profileUser.name })
		).toBeVisible();
		await expect(
			page.getByRole('definition').filter({ hasText: profileUser.email })
		).toBeVisible();
	});
