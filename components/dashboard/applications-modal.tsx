...
export function ApplicationsModal({ isOpen, onClose, roastRequest, applications }: ApplicationsModalProps) {
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-6">
          ...
          {pendingApplications.length === 0 && selectedApplicationsData.length === 0 ? (
                          ...
                          <div className="flex items-center justify-between">
                            <div className="flex items-center gap-3">
                              ...
                              <Badge className="bg-green-100 text-green-800 text-xs">
                                {application.status === 'auto_selected' ? 'Auto' : 'Sélectionné'}
                              </Badge>
                            </div>
                            <div className="flex items-center gap-4 text-sm text-gray-600">
                              <div className="flex items-center gap-1">
                                <Star className="w-3 h-3 text-yellow-500" />
                                <span>{application.roaster.roasterProfile?.rating.toFixed(1) || '0.0'}</span>
                              </div>
                              <div className="flex items-center gap-1">
                                <Award className="w-3 h-3 text-blue-500" />
                                <span>{application.roaster.roasterProfile?.completedRoasts || 0}</span>
                              </div>
                            </div>
                          </div>
                        </CardContent>
                  ...
                  <div className="flex items-center justify-between mb-4">
                    ...
                    {canSelect && selectedApplications.length > 0 && (
                      <Button 
                        ...
                        size="sm"
                      >
                        {isLoading ? 'Sélection...' : `Sélectionner ${selectedApplications.length}`}
                      </Button>
                    )}
                  </div>
                            ...
                            <div className="flex-1">
                              ...
                              <div className="mb-3">
                                <div className="flex flex-wrap gap-1">
                                  {application.roaster.roasterProfile?.specialties.slice(0, 3).map((specialty) => {
                                  ...
                                  }) || <span className="text-gray-500 text-xs">Aucune spécialité</span>}
                                </div>
                              </div>
                              ...
                              {application.motivation && (
                                <div className="bg-gray-50 rounded-lg p-3">
                                  <p className="text-xs text-gray-700 italic">"{application.motivation}"</p>
                                </div>
                              )}
                            </div>
                          </div>
