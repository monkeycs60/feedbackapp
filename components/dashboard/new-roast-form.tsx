...
} from '@/lib/actions/target-audiences';
...
const formSchema = z.object({
	...
	targetAudienceIds: z
		.array(z.string())
		.min(1, 'Sélectionne au moins une audience cible')
		.max(2, 'Maximum 2 audiences cibles'),
	customTargetAudience: z
		.object({
	...
	feedbacksRequested: z
		.number()
		.min(1, 'Au moins 1 feedback')
		.max(20, 'Maximum 20 feedbacks'),
	selectedDomains: z
		.array(
			z.object({
				id: z.string(),
...
export function NewRoastForm() {
	...
	const createAudienceMutation = useMutation({
		...
		onSuccess: (newAudience) => {
			...
			const updatedIds = targetAudienceIds.map((id) =>
				id === 'custom' ? newAudience.id : id
			);
			form.setValue('targetAudienceIds', updatedIds);
			form.unregister('customTargetAudience.name');
	...
	const calculatePrice = () => {
		...
		const additionalQuestionsPrice =
			selectedDomains.reduce((total, domain) => {
				const additionalQuestions = domain.questions.filter(
					(q) => !q.isDefault
				).length;
				return total + additionalQuestions;
			}, 0) * PRICING.ADDITIONAL_QUESTION;
		const feedbacksPrice =
			(domainsPrice + additionalQuestionsPrice) * feedbacksRequested;
		form.setValue('totalPrice', feedbacksPrice);
		return feedbacksPrice;
	};

	const toggleDomain = (domainId: FocusArea) => {
		const isDomainAlreadySelected = selectedDomains.some(
			(d) => d.id === domainId
		);
		...
		if (isDomainAlreadySelected) {
			...
			if (focusArea) {
				const defaultQuestions: DomainQuestion[] = focusArea.questions.map(
					(text, index) => ({
		...
		const updated = selectedDomains.map((domain) => {
			if (domain.id === domainId) {
				const maxOrder = Math.max(
					...domain.questions.map((q) => q.order),
					-1
				);
	...
	const removeQuestion = (domainId: FocusArea, questionId: string) => {
		const updated = selectedDomains.map((domain) => {
			if (domain.id === domainId) {
				const remainingQuestions = domain.questions.filter(
					(q) => q.id !== questionId
				);
	...
	const onSubmit = async (data: FormData) => {
		...
		try {
			// Remove customTargetAudience from the data
			const { customTargetAudience, ...cleanData } = data;

			// Transformer les données pour correspondre à l'ancien format
			const transformedData = {
				...cleanData,
				focusAreas: cleanData.selectedDomains.map((d) => d.id),
				maxPrice: cleanData.totalPrice,
				isUrgent: false, // Plus d'urgence
				selectedDomains: cleanData.selectedDomains, // Envoyer aussi les domaines avec questions
				coverImage: cleanData.coverImage,
				feedbacksRequested: cleanData.feedbacksRequested,
			};
							...
							<CardContent className='space-y-4'>
								...
								<div>
									...
									{form.formState.errors.appUrl && (
										<p className='text-red-500 text-sm mt-1'>
											{form.formState.errors.appUrl.message}
										</p>
									)}
								</div>
								...
								<div>
									...
									<p className='text-sm text-gray-600 mb-2'>
										Ajoute une capture d&apos;écran pour illustrer ton
										app
									</p>
									<ImageUpload
							...
							<CardContent className='space-y-4'>
								<div>
									...
									<div className='flex justify-between text-sm text-gray-500 mt-1'>
										<span>
											Min 50 caractères pour un feedback de qualité
										</span>
								...
								<div>
									...
									<div className='mt-2 space-y-2 max-h-48 overflow-y-auto border rounded-lg p-3'>
										...
										<div className='flex items-center space-x-2'>
											<Checkbox
												id='custom-audience'
												checked={targetAudienceIds.includes(
													'custom'
												)}
												onCheckedChange={() =>
													handleAudienceToggle('custom')
												}
												disabled={
													targetAudienceIds.length >= 2 &&
													!targetAudienceIds.includes('custom')
												}
											/>
											<Label
									...
									{targetAudienceIds.length > 0 && (
										<p className='text-xs text-gray-600 mt-1'>
											{targetAudienceIds.length}/2 audiences
											sélectionnées
										</p>
									)}
								...
								{showCustomAudience && (
									<div className='space-y-4 p-4 bg-gray-50 rounded-lg'>
										<div>
											<Label htmlFor='customAudienceName'>
												Nom de l'audience
											</Label>
											...
											{form.formState.errors.customTargetAudience
												?.name && (
												<p className='text-red-500 text-sm mt-1'>
													{
														form.formState.errors
															.customTargetAudience.name.message
													}
												</p>
											)}
											<p className='text-xs text-gray-600 mt-1'>
												Cette audience sera ajoutée à la liste pour
									...
									{FOCUS_AREAS.map((area) => {
										...
										return (
											<button
												...
												type='button'
												onClick={() =>
													toggleDomain(area.id as FocusArea)
												}
						...
						{selectedDomains.length > 0 && (
							<Card>
								<CardHeader>
									...
									<p className='text-sm text-gray-600'>
										Personnalise tes questions (+1€ par question
										supplémentaire)
									</p>
								</CardHeader>
								<CardContent className='space-y-6'>
									{selectedDomains.map((selectedDomain) => {
										const domainInfo = FOCUS_AREAS.find(
											(area) => area.id === selectedDomain.id
										);
											...
											<div
												...
												<div className='flex items-center justify-between mb-4'>
													...
													<Button
														...
														onClick={() =>
															removeDomain(
																selectedDomain.id as FocusArea
															)
														}>
														<X className='h-4 w-4' />
													</Button>
												...
												<div className='space-y-3'>
													<Reorder.Group
														axis='y'
														values={selectedDomain.questions.sort(
															(a, b) => a.order - b.order
														)}
						...
						{selectedDomains.length > 0 && (
							<Card>
								<CardHeader>
									...
									<p className='text-sm text-gray-600'>
										Plus de feedbacks = plus d&apos;opinions diverses
										et complètes
									</p>
								</CardHeader>
								<CardContent className='space-y-4'>
									<div className='space-y-3'>
										<Label htmlFor='feedbacksRequested'>
											{feedbacksRequested} feedback
											{feedbacksRequested > 1 ? 's' : ''} demandé
											{feedbacksRequested > 1 ? 's' : ''}
										</Label>
										<input
									...
									<div className='bg-blue-50 rounded-lg p-3 border border-blue-200'>
										<h4 className='font-medium text-blue-900 mb-2'>
											Avantages des feedbacks multiples :
										</h4>
										<ul className='text-sm text-blue-800 space-y-1'>
											<li>
												•{' '}
												<strong>Diversité d&apos;expertise</strong>{' '}
												: UX, dev, business, marketing
											</li>
											<li>
												• <strong>Points de consensus</strong> :
												Identifier les vrais problèmes
											</li>
											<li>
												• <strong>Perspectives variées</strong> :
												Débutants vs experts
											</li>
											<li>
							...
							<div className='space-y-2'>
								{selectedDomains.length > 0 && (
									<div className='flex justify-between'>
										...
										<span>
											{selectedDomains.length * PRICING.DOMAIN_PRICE}
											€
										</span>
									</div>
								)}
								...
								{selectedDomains.reduce(
									(total, domain) =>
										total +
										domain.questions.filter((q) => !q.isDefault)
											.length,
									0
								) > 0 && (
									<div className='flex justify-between'>
