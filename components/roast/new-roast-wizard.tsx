...
export function NewRoastWizard({
	targetAudiences,
	className = '',
}: NewRoastWizardProps) {
		...
		switch (stepIndex) {
			...
			case 2: // Questions
				...
				const hasValidDomains = selectedDomains.some((domainId) => {
					const domainConfig = FOCUS_AREAS.find(
						(area) => area.id === domainId
					);
					return (
						domainConfig?.questions && domainConfig.questions.length > 0
					);
				});
				...
				{confirmSubmit && (
					<Alert className='mb-6 border-red-200 bg-red-50'>
						...
						<AlertDescription className='text-red-800'>
							...
							publier votre roast. Cliquez à nouveau sur "Confirmer et
							créer le roast" pour finaliser.
						</AlertDescription>
					</Alert>
				)}
			...
			<Card>
				...
				<CardContent className='space-y-6'>
					...
					<div className='space-y-2'>
						...
						<div className='flex justify-between text-xs text-muted-foreground'>
							<span>
								Plus c'est détaillé, meilleurs seront les retours
							</span>
					...
					<div className='space-y-3'>
						...
						/>
						<p className='text-xs text-muted-foreground'>
							Cette image sera visible sur la marketplace pour attirer
							les roasters
						</p>
					</div>
					...
					<div className='space-y-4'>
							...
							<input
								...
								style={{
									background: `linear-gradient(to right, #3b82f6 0%, #3b82f6 ${
										((watchedValues.feedbacksRequested || 2) - 1) *
										11.11
									}%, #e5e7eb ${
										((watchedValues.feedbacksRequested || 2) - 1) *
										11.11
						...
						<p className='text-xs text-muted-foreground'>
							Plus vous avez de retours, plus vous aurez de perspectives
							différentes
						</p>
						{errors.feedbacksRequested && (
							<p className='text-red-500 text-sm'>
								{errors.feedbacksRequested.message}
							</p>
						)}
					</div>
					...
					<div className='flex items-center space-x-3 p-4 bg-gradient-to-r from-yellow-50 to-orange-50 border border-yellow-200 rounded-lg'>
						...
						<div className='flex-1'>
							...
							<p className='text-xs text-orange-700 mt-1'>
								• Mis en avant dans l'algorithme de la marketplace
								<br />
								• Publication immédiate (pas de collecte de 24h)
								<br />• Rémunération bonus pour attirer les meilleurs
								roasters
							</p>
						</div>
					</div>
		...
		<div className='space-y-4'>
			...
			<div className='space-y-3'>
				...
				{errors.targetAudienceIds && (
					<p className='text-red-500 text-sm'>
						{errors.targetAudienceIds.message}
					</p>
				)}
			</div>
				...
				<CardContent className='space-y-6'>
					...
					<div className='space-y-4'>
						...
						<div className='bg-gray-50 rounded-lg p-4 space-y-3'>
							...
							<div>
								<span className='font-medium'>Roasters demandés :</span>{' '}
								{watchedValues.feedbacksRequested}
							</div>
					...
					<div className='bg-blue-50 border border-blue-200 rounded-lg p-4'>
						...
						<p className='text-sm text-blue-700'>
							...
							{watchedValues.isUrgent
								...
								: 'mis en collecte de candidatures pendant 24h'}
							. Les roasters recevront une notification et pourront
							postuler.
						</p>
					</div>
				</CardContent>
